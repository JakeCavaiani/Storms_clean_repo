...
MOOS.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_MOOS)
...
FRCH.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_FRCH)
...
CARI.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_CARI)
...
POKE.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_POKE)
...
VAUL.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_VAUL)
...
STRT.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena, data = HI_FI_NO3_STRT)
...
Plot_1 <- plot_summs(CARI.model, FRCH.model,
           MOOS.model, POKE.model, STRT.model, 
           VAUL.model, scale = TRUE,
...
scale_y_discrete(labels= c("Seasonality", 
                             ...
                             "Storm precipitation")) +
  geom_text(data = no3hireg, aes(x = -0.3, y = c(1.5, 1.35, 1.2, 1.05, 0.9, 0.75), color = site.ID, size = 15, hjust = 0,
           label = paste("R[adj]^2", "==", round(adj.r.squared, 2))), parse = TRUE) +
  theme_bw() +
  ggtitle(vn) +
