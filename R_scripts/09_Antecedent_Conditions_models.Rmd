...
```
### ***TKH the above is replaced by the section below (after normalizing) and can be deleted?
### ***TKH Why is intensity 1 for all of 2022? 
...
###***TKH: correlated pairs to avoid in same model- intensity & stormppt, month & 3month, week & month, TSC & 3month
...
MOOS.no3.model <- lm(Hyst_index ~ StormPrecip + PrecipMonth + TimeSinceChena + Intensity, data = HI_FI_NO3_MOOS)

vif(MOOS.no3.model)
plot(MOOS.no3.model)
summary(MOOS.no3.model)
