...
pr <- ggpredict(MOOS.model)
plot(pr, add.data = TRUE)

```

```{r}
# partial residual plot test to get each site by color 
mo <- ggpredict(MOOS.model)
plot(mo, add.data = TRUE)

fr <- ggpredict(FRCH.model)
plot(fr, add.data = TRUE)

ca <- ggpredict(CARI.model)
plot(ca, add.data = TRUE)

po <- ggpredict(POKE.model)
plot(po, add.data = TRUE)

va <- ggpredict(VAUL.model)
plot(va, add.data = TRUE)

st <- ggpredict(STRT.model)
plot(st, add.data = TRUE)
...
```{r}
M1 <- lmer(Beta_index ~ StormPrecip + PrecipWeek + PrecipMonth + TimeSinceChena + (1|site.ID), data = HI_FI_NO3)
pr <- ggpredict(M1)
...
```
```{r}
M1 <- lmer(Beta_index ~ StormPrecip + PrecipWeek + PrecipMonth + TimeSinceChena + (1|site.ID), data = HI_FI_fDOM)
pr <- ggpredict(M1)
...
```
```{r}
M1 <- lmer(Beta_index ~ StormPrecip + PrecipWeek + PrecipMonth + TimeSinceChena + (1|site.ID), data = HI_FI_SPC)
pr <- ggpredict(M1)
...
plot(M1, rstudent(.) ~ hatvalues(.))
```
qq norm plot has some crazy tails 
...
```
```{r}
M1 <- lmer(Beta_index ~ StormPrecip + PrecipWeek + PrecipMonth + TimeSinceChena + (1|site.ID), data = HI_FI_turb)
pr <- ggpredict(M1)
...
plot(M1, rstudent(.) ~ hatvalues(.))
```
residuals are normally distributed and qqnorm plot got some big tails 


# plots for thesis # 
```{r}
ggarrange(no3_hi, fdom_hi,
          no3_beta, fDOM_beta,
          ncol = 2, nrow = 2,
          common.legend = TRUE,
          labels = c("A)", "B)",
...
ggsave(path = "~/Documents/Storms_clean_repo/plots/Coefficient_Plots", 
       filename = "NO3_fDOM.pdf",
       width = 10, height = 10)
