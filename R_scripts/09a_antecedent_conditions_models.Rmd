...
pairs <- AMC %>% filter(site.ID == "MOOS" & response_var == "NO3") %>%
        ggpairs(columns = c("precip.week_z", "precip.month_z", "ThreeMonth_z", "Intensity_z", "TimeSinceChena", "Hyst_index"), mapping = aes(color = as.factor(year)))
        
ggsave(pairs, file = "HI_NO3_MOOS_pairs.pdf",
       path = here("Output_from_analysis", "09_Antecedent_Conditions_models", "plots"),
       width = 10, height = 10, units = "in")

mod1 <- AMC %>% filter(site.ID == "MOOS" & response_var == "NO3") %>%
                lmer(Hyst_index ~ precip.week_z + precip.month_z + TimeSinceChena + Intensity_z + (1|year), data = .)

vif(mod1)
qqnorm(resid(mod1,  type = "pearson"))
