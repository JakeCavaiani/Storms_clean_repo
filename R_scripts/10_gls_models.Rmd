...
library(ggpubr)
pwc <- mean_daily_2018 %>%
  pairwise_t_test(dailyfDOM ~ site.ID, p.adjust.method = "bonferroni")
pwc

pwc <- pwc %>% add_xy_position(x = "site.ID")
res.aov <- mean_daily_2018 %>% anova_test(dailyfDOM ~ site.ID)
res.aov
ggboxplot(mean_daily_2018, x = "site.ID", y = "dailyfDOM") +
...
anova <- aov(logDailyTurb ~ site.ID, data = mean_daily_2021)
summary(anova)
tukey <- TukeyHSD(anova)
print(tukey)
cld <- multcompLetters4(anova, tukey)
print(cld)
cld <- as.data.frame.list(cld$site.ID)

Tk <- mean_daily_2021 %>% 
  group_by(site.ID) %>% 
  summarise(mean = mean(dailyTurb, na.rm = TRUE), 
            quant = quantile(dailyTurb, na.rm = TRUE), probs = 0.75) %>% 
  arrange(desc(mean))
Tk <- Tk[!duplicated(Tk$mean), ]
Tk$cld <- cld$Letters



mean_daily_2021$cld <- NA

mean_daily_2021 <- mean_daily_2021 %>% 
  mutate(across(c(cld),
                ~ifelse(site.ID == "CARI", "e", .)))

mean_daily_2021 <- mean_daily_2021 %>% 
  mutate(across(c(cld),
                ~ifelse(site.ID == "VAUL", "a", .)))

mean_daily_2021 <- mean_daily_2021 %>% 
  mutate(across(c(cld),
                ~ifelse(site.ID == "FRCH", "b", .)))
...
ggplot(mean_daily_2021, aes(x = site.ID, y = dailyTurb, fill = site.ID)) +
  ...
  ylab("") +
  theme_classic() +
  geom_text(data = Tk, aes(label = cld, x = site.ID, y = quant),
            vjust = -5, hjust = -1, size = 3)
