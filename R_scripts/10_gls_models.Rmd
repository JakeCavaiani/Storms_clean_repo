...
write.csv(mean_daily_site, "~/GitHub/Storms_clean_repo/Output_from_analysis/10_gls_models/mean_daily_site.csv")
...
pacf(Efulla, na.action=na.pass)
```
###***TKH: ARMA(1,1) is good here
# JSC: using the ARMA (1,1) model 
...
```
# For 2018 - nitrate data - None are significantly different
F-value: 0.05994
p-value: 0.9418
Denom DF: 212
...
plot(print(confint(site.ID.comp)))
tab_model(fDOM.mod.arma.1.1)
anova(fDOM.mod.arma.1.1)


```
This shows that there is overlap in CI for each comp (FRCH:CARI, MOOS:CARI, MOOS:FRCH)
F-value: 2.8893
p-value: 0.0575
Denon DF: 245

###***TKH: MOOS & CARI are significantly different
# JSC: on 9/12/2025 - I don't see how these are significantly different
...
```
This shows no significance for any of the sites as CI overlap 0 for all comps
F-value: 0.35597
p-value: 0.701
Denom DF: 185
...
# Found some rows in the data frame that had duplicated julian days. Fixing it here below
# Adjust julian date values using case_when for specific conditions
mean_daily_2019 <- mean_daily_2019 %>%
  mutate(julian = case_when(
    site.ID == "STRT" & day == "2019-08-28" ~ 239,
    site.ID == "STRT" & day == "2019-08-29" ~ 240,
    site.ID == "STRT" & day == "2019-08-30" ~ 241,
    site.ID == "STRT" & day == "2019-08-31" ~ 242,
    site.ID == "STRT" & day == "2019-09-04" ~ 246,
    site.ID == "STRT" & day == "2019-09-05" ~ 247,
    site.ID == "STRT" & day == "2019-09-06" ~ 248,
    site.ID == "STRT" & day == "2019-09-07" ~ 249,
    TRUE ~ julian  # Keep original value if none of the conditions match
  ))
...
aictab(cand.set = models) # turb.mod.arma.1.2 is the best model
```

# generalized linear hypotheses
```{r}
site.ID.comp <- glht(turb.mod.arma.1.2, linfct = mcp(site.ID = "Tukey"))
summary(site.ID.comp)
summary(turb.mod.arma.1.2)
intervals(turb.mod.arma.1.2)
plot(print(confint(site.ID.comp)))
anova(turb.mod.arma.1.2)
...
(Intercept)     1 1091.5796  <.0001
site.ID         5   35.7736  <.0001
...
Denom. DF: 610 
            numDF   F-value p-value
(Intercept)     1 234.01095  <.0001
site.ID         5   2.79659  0.0165
...
Denom. DF: 564 
            numDF  F-value p-value
(Intercept)     1 710.5180  <.0001
site.ID         5   3.3979  0.0049
...
Denom. DF: 555 
            numDF  F-value p-value
(Intercept)     1 327.0362  <.0001
site.ID         5   2.2971   0.044
...
Denom. DF: 557 
            numDF   F-value p-value
(Intercept)     1 127.96977  <.0001
site.ID         5  10.04653  <.0001
...
Denom. DF: 553 
            numDF   F-value p-value
(Intercept)     1 1921.5616  <.0001
site.ID         5   12.7905  <.0001
...
Denom. DF: 563 
            numDF  F-value p-value
(Intercept)     1 394.0568  <.0001
site.ID         5   7.8349  <.0001

# 2022 ===========================================================================
```{r}
mean_daily_2022 <- subset(mean_daily, year == "2022")
mean_daily_2022$site.ID <- as.factor(mean_daily_2022$site.ID)

ggplot(mean_daily_2022, aes(x = julian, y = dailyNO3, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

no3.2022 <- ggplot(mean_daily_2022, aes(x = site.ID, y = dailyNO3, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A"), "Site") +
  xlab("Site") +
  ylab(vn) +
...
no3.mod.gls <- gls(dailyNO3 ~ site.ID, 
               na.action = na.omit, 
               data = mean_daily_2022, 
               correlation = corAR1(form = ~ julian|site.ID, value = 0.5))
```
...
```{r}
Ear1<-residuals(no3.mod.gls, type="normalized")
I1<-!is.na(mean_daily_2022$dailyNO3)
Efulla<-vector(length = length(mean_daily_2022$dailyNO3))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
summary(NO3.mod.arma.1.1)
intervals(NO3.mod.arma.1.1)
plot(print(confint(NO3.mod.arma.1.1)))
anova(NO3.mod.arma.1.1)
...
### fDOM ### 
```{r}
mean_daily_2022 <- mean_daily_2022 %>%
  mutate(across(c(dailyfDOM), 
                ~ifelse(dailyfDOM < 0, NA, .)))
...
ggplot(mean_daily_2022, aes(x = julian, y = dailyfDOM, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

fDOM.2022 <- ggplot(mean_daily_2022, aes(x = site.ID, y = dailyfDOM, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A"), "Site") +
  xlab("Site") +
  ylab("") +
...
# log transformed 
```{r}
mean_daily_2022$logDailyfDOM <- log(mean_daily_2022$dailyfDOM)
...
```{r}
Ear1<-residuals(fDOM.mod.ar1, type="normalized")
I1<-!is.na(mean_daily_2022$dailyfDOM)
Efulla<-vector(length = length(mean_daily_2022$dailyfDOM))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
Denom. DF: 589 
            numDF  F-value p-value
(Intercept)     1 89.04169  <.0001
site.ID         5  6.03491  <.0001
...
### SPC ### 
```{r}
ggplot(mean_daily_2022, aes(x = julian, y = dailySPC, color = site.ID)) +
  geom_line() +
    scale_color_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

spc.2022 <- ggplot(mean_daily_2022, aes(x = site.ID, y = dailySPC, fill = site.ID)) +
  geom_boxplot() +
    scale_fill_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A"), "Site") +
  xlab("Site") +
  ylab("") + 
...
```{r}
Ear1<-residuals(SPC.mod.ar1, type="normalized")
I1<-!is.na(mean_daily_2022$dailySPC)
Efulla<-vector(length = length(mean_daily_2022$dailySPC))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
```{r}
models <- list(SPC.mod.arma.1.1, SPC.mod.arma.2.2)
aictab(cand.set = models) # SPC.mod.arma.2.2 is the best model
```

# generalized linear hypotheses
```{r}
site.ID.comp <- glht(SPC.mod.arma.1.2, linfct = mcp(site.ID = "Tukey"))
summary(site.ID.comp)
summary(SPC.mod.arma.1.2)
plot(print(confint(site.ID.comp)))
anova(SPC.mod.arma.1.2)

```

Denom. DF: 613 
            numDF  F-value p-value
(Intercept)     1 4.501619  0.0343
site.ID         5 0.036712  0.9993


### Turb ### 
```{r}
ggplot(mean_daily_2022, aes(x = julian, y = dailyTurb, color = site.ID)) +
  geom_line() +
    scale_color_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

turb.2022 <- ggplot(mean_daily_2021, aes(x = site.ID, y = dailyTurb, fill = site.ID)) +
  geom_boxplot() +
    scale_fill_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A"), "Site") +
  xlab("Site") +
  ylab("") +
...
```{r}
mean_daily_2022$logDailyTurb <- log(mean_daily_2022$dailyTurb)
...
```{r}
Ear1<-residuals(turb.mod.ar1.log, type="normalized")
I1<-!is.na(mean_daily_2022$logDailyTurb)
Efulla<-vector(length = length(mean_daily_2022$logDailyTurb))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
```{r}
models <- list(turb.mod.arma.1.1, turb.mod.arma.1.2, turb.mod.arma.2.1, turb.mod.arma.2.2)
aictab(cand.set = models) # turb.mod.arma.2.2 is the best model
```
...
Denom. DF: 604 
            numDF   F-value p-value
(Intercept)     1 151.69187  <.0001
site.ID         5   8.45034  <.0001
...
# CorAR1 structure #
```{r}
mean_daily_2015 <- subset(mean_daily, year == "2015")
mean_daily_2015$site.ID <- as.factor(mean_daily_2015$site.ID)

ggplot(mean_daily_2015, aes(x = julian, y = dailyNO3, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#FF7F00", "#A6761D")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

ggplot(mean_daily_2015, aes(x = site.ID, y = dailyNO3, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#FF7F00", "#A6761D"), "Site") +
  theme_classic() + 
  xlab("Site") +
  ylab(vn) 
...
### no autocorrelation structure
```{r}
no3.mod.gls.2015 <- gls(dailyNO3 ~ site.ID, 
               na.action = na.omit, 
               data = mean_daily_2015)

```
# diagnostic plots 
```{r}
plot(no3.mod.gls.2015, resid(., type = "p") ~ fitted(.), abline = 0)

plot(no3.mod.gls.2015, dailyNO3 ~ fitted(.) | site.ID, abline = c(0,1))

qqnorm(no3.mod.gls.2015, ~ resid(., type = "p"), abline = c(0, 1))
```
Our normality looks pretty good


# corAR1 structure 
```{r}
no3.mod.ar1.2015 <- gls(dailyNO3 ~ site.ID, 
                        na.action = na.omit, 
                        data = mean_daily_2015, 
                        correlation = corAR1(form = ~ julian | site.ID, value = 0.5),
                        control = list(maxIter = 100))

summary(no3.mod.ar1.2015)

```

# ACF plot 
```{r}
Ear1<-residuals(no3.mod.ar1.2015, type="normalized")
I1<-!is.na(mean_daily_2015$dailyNO3)
Efulla<-vector(length = length(mean_daily_2015$dailyNO3))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
no3.mod.arma.1.1 <- gls(dailyNO3 ~ site.ID, 
               na.action = na.omit, 
               data = mean_daily_2015, 
               correlation = corARMA(form = ~ julian|site.ID, p = 0, q = 1))
...
# AIC 
```{r}
models <- list(no3.mod.ar1.2015,no3.mod.arma.1.1, no3.mod.arma.1.2, no3.mod.arma.2.1, no3.mod.arma.2.2)
aictab(cand.set = models) # no3.mod.arma.1.1 is the best model
```
# generalized linear hypotheses
```{r}
...
Denom. DF: 146 
            numDF   F-value p-value
(Intercept)     1 1967.2220  <.0001
site.ID         1   14.6102   2e-04

### fDOM ### 
```{r}
ggplot(mean_daily_2015, aes(x = julian, y = dailyfDOM, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#FF7F00", "#A6761D")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

fDOM.2015 <- ggplot(mean_daily_2015, aes(x = site.ID, y = dailyfDOM, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#FF7F00", "#A6761D"), "Site") +
  xlab("") +
  ylab("Daily fDOM (QSU)") +
...
# gls  
```{r}
fDOM.mod.2015 <- gls(dailyfDOM ~ site.ID, 
               na.action = na.omit, 
               data = mean_daily_2015)
...
# diagnostic plots 
```{r}
plot(fDOM.mod.2015, resid(., type = "p") ~ fitted(.), abline = 0)

plot(fDOM.mod.2015, dailyfDOM ~ fitted(.) | site.ID, abline = c(0,1))

qqnorm(fDOM.mod.2015, ~ resid(., type = "p"), abline = c(0, 1))
...
```{r}
Ear1<-residuals(fDOM.mod.ar1, type="normalized")
I1<-!is.na(mean_daily_2015$dailyfDOM)
Efulla<-vector(length = length(mean_daily_2015$dailyfDOM))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
```
FRCH and MOOS Significantly different from each other 
Denom. DF: 172 
            numDF   F-value p-value
(Intercept)     1 232.39049  <.0001
site.ID         1  10.23507  0.0016


### SPC ### 
```{r}
ggplot(mean_daily_2015, aes(x = julian, y = dailySPC, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#FF7F00", "#A6761D")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

spc.2015 <- ggplot(mean_daily_2015, aes(x = site.ID, y = dailySPC, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#FF7F00", "#A6761D"), "Site") +
  xlab("") +
  ylab("Daily SPC (µS/cm)") +
...
```{r}
Ear1<-residuals(SPC.mod.ar1, type="normalized")
I1<-!is.na(mean_daily_2015$dailySPC)
Efulla<-vector(length = length(mean_daily_2015$dailySPC))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
pacf(Efulla, na.action=na.pass)
```
Still autocorrelation at the 8th lag 
...
```
This shows that MOOS is not significantly different from FRCH
Denom. DF: 164 
            numDF  F-value p-value
(Intercept)     1 342.7039  <.0001
site.ID         1   3.7403  0.0548

### Turb ### 
```{r}
ggplot(mean_daily_2015, aes(x = julian, y = dailyTurb, color = site.ID)) +
  geom_line() +
  scale_color_manual(values=c("#FF7F00", "#A6761D")) +
  facet_wrap(~site.ID, scales = "free") +
  theme_classic()

turb.2015 <- ggplot(mean_daily_2015, aes(x = site.ID, y = dailyTurb, fill = site.ID)) +
  geom_boxplot() +
  scale_fill_manual(values=c("#FF7F00", "#A6761D"), "Site") +
  xlab("") +
  ylab("Daily Turbidity (FNU)") +
...
# gls 
```{r}
mean_daily_2015$logDailyTurb <- log(mean_daily_2015$dailyTurb)
turb.mod.log <- gls(logDailyTurb ~ site.ID, 
               na.action = na.omit, 
               data = mean_daily_2015)
...
```{r}
Ear1<-residuals(turb.mod.ar1.log, type="normalized")
I1<-!is.na(mean_daily_2015$logDailyTurb)
Efulla<-vector(length = length(mean_daily_2015$logDailyTurb))
Efulla<-NA
Efulla[I1]<-Ear1
acf(Efulla, na.action=na.pass)
pacf(Efulla, na.action=na.pass)
...
pacf(Efulla, na.action=na.pass)
```
This looks like there is still autocorrelation in the later lags 
...
# AIC 
```{r}
models <- list(turb.mod.log, turb.mod.arma.1.1, turb.mod.arma.1.2, turb.mod.arma.2.1, turb.mod.arma.2.2)
aictab(cand.set = models) # turb.mod.arma.1.1 is the best model
```
...
```
MOOS and FRCH are significantly different
Denom. DF: 164 
            numDF  F-value p-value
(Intercept)     1 87.14900  <.0001
site.ID         1  4.89842  0.0283

### MEAN DAILY CHEM PLOTS BY YEAR - 1/5/2026 ###
```{MEAN DAILY CHEM PLOTS BY YEAR}
# NO3 ####
vn = expression(paste(N*O[3]^"-"*(μM)))
vn <- expression(N*O[3]^-*mu*"M")
vn <- expression(paste(N*O[3]^"-" * " (" * mu * "M)"))

mean_daily_new <- mean_daily

### UPDATED PLOT SYMBOLOGY ###
# PF extent
# POKE - 25%
# CARI - 29%
# STRT - 30%
...
annotations <- data.frame(
  ...
  site.ID = c("CARI", "FRCH", "VAUL", "MOOS", "POKE", "STRT",         
              "CARI", "FRCH", "VAUL", "MOOS", "POKE", "STRT",
              "CARI", "FRCH", "VAUL", "MOOS", "POKE", "STRT"),        
  Label = c("a", "a", "c", "b", "a", "ad",                           
            "b", "a", "ab", "ab", "ab", "a",
            "ac", "ac", "b", "b", "a", "bc"),                       
  x_position = c(2, 4, 6, 5, 1, 3,                                   
                 2, 4, 6, 5, 1, 3,
                 2, 4, 6, 5, 1, 3),                                  
  y_position = c(35, 55, 50, 35, 40, 40,                
                 45, 40, 55, 40, 55, 35,
                 35, 50, 50, 32, 50, 40)                
)

no3 <- ggplot(mean_daily_new, aes(x = site.ID, y = dailyNO3, pattern = Burn, fill = site.ID)) +
  ...
  scale_fill_manual(values=c("#3288BD","#FF7F00", "#A6761D", "#6A3D9A", "#66C2A5", "#E7298A"), guide = "none") +
  facet_wrap(~year) +
...
ggsave(no3, path = here("plots", "Chems_year"),
       file = paste0("NO3_facet_", Sys.Date(), ".pdf"),
       width = 12, height = 8, units = "in")

ggsave(no3, path = here("plots", "Chems_year"),
       file = paste0("NO3_facet_", Sys.Date(), ".jpg"),
       width = 12, height = 8, units = "in")
...
annotations <- data.frame(
  ...
  x_position = c(2, 4, 6, 5, 1, 3,                                   
                 ...
                 2, 4, 6, 5, 1, 3,
                 2, 4, 6, 5, 1, 3),                                  
  y_position = c(200, 200, 200, 250, 200, 200,                
                 200, 200, 270, 200, 150, 150, 
                 200, 150, 275, 200, 150, 150, 
                 200, 150, 310, 200, 80, 150, 
                 110, 150, 315, 275, 80, 110)                
)

fdom <- ggplot(mean_daily_new, aes(x = site.ID, y = dailyfDOM, pattern = Burn, fill = site.ID)) +
  geom_boxplot(width = .25,
                 outlier.colour = NA,
                 alpha = 0.5) +
   geom_boxplot_pattern(position = position_dodge(preserve = "single"), 
...
ggsave(fdom, path = here("plots", "Chems_year"),
       file = paste0("fDOM_facet_", Sys.Date(), ".pdf"),
       width = 12, height = 8, units = "in")

ggsave(fdom, path = here("plots", "Chems_year"),
       file = paste0("fDOM_facet_", Sys.Date(), ".jpg"),
       width = 12, height = 8, units = "in")
...
annotations <- data.frame(
  ...
  x_position = c(2, 4, 6, 5, 1, 3,                                   
                 ...
                 2, 4, 6, 5, 1, 3,
                 2, 4, 6, 5, 1, 3),                                  
  y_position = c(400, 300, 200, 300, 200, 200,                
                 300, 200, 600, 220, 200, 200, 
                 300, 200, 500, 300, 150, 150, 
                 300, 200, 400, 220, 75, 200, 
                 150, 200, 400, 300, 75, 150)                
)

spc <- ggplot(mean_daily_new, aes(x = site.ID, y = dailyfDOM, pattern = Burn, fill = site.ID)) +
  geom_boxplot(width = .25,
                 outlier.colour = NA,
                 alpha = 0.5) +
   geom_boxplot_pattern(position = position_dodge(preserve = "single"), 
...
ggsave(spc, path = here("plots", "Chems_year"),
       file = paste0("SPC_facet_", Sys.Date(), ".pdf"),
       width = 12, height = 8, units = "in")

ggsave(spc, path = here("plots", "Chems_year"),
       file = paste0("spc_facet_", Sys.Date(), ".jpg"),
       width = 12, height = 8, units = "in")
...
annotations <- data.frame(
  ...
  x_position = c(2, 4, 6, 5, 1, 3,                                   
                 ...
                 2, 4, 6, 5, 1, 3,
                 2, 4, 6, 5, 1, 3),                                  
  y_position = c(100, 100, 100, 100, 100, 100,                
                 210, 210, 300, 220, 150, 150, 
                 275, 200, 300, 220, 120, 200, 
                 240, 200, 330, 210, 75, 200, 
                 100, 200, 350, 300, 75, 110)                
)

turb <- ggplot(mean_daily_new, aes(x = site.ID, y = dailyfDOM, pattern = Burn, fill = site.ID)) +
  geom_boxplot(width = .25,
                 outlier.colour = NA,
                 alpha = 0.5) +
   geom_boxplot_pattern(position = position_dodge(preserve = "single"), 
...
ggsave(turb, path = here("plots", "Chems_year"),
       file = paste0("Turb_facet_", Sys.Date(), ".pdf"),
       width = 12, height = 8, units = "in")

ggsave(turb, path = here("plots", "Chems_year"),
       file = paste0("turb_facet_", Sys.Date(), ".jpg"),
       width = 12, height = 8, units = "in")
...
# I might adjust them to this style - 02/21/2023
mean_daily_new <- mean_daily_new %>% 
...
# NO3 #
# 2015 #
plot.2015 <- mean_daily_new %>% 
filter(datetimeAK < "2016-01-01" & datetimeAK > "2015-01-01") %>%
ggplot(aes(x = site.ID, y = dailyNO3, pattern = PF, fill = site.ID)) +
  ...
  scale_fill_manual(values=c("#FF7F00", "#A6761D")) +
  xlab("") +
  ylab(vn) +
  ...
  theme(axis.text.x=element_text(size=20, angle = 90, vjust = 0.5, hjust=1),
        ...
        strip.text = element_text(size = 20))
# 2018 #
plot.2018 <- mean_daily_new %>% 
filter(datetimeAK < "2019-01-01" & datetimeAK > "2018-01-01") %>%
ggplot(aes(x = site.ID, y = dailyNO3, pattern = PF, fill = site.ID)) +
  facet_wrap(~year, nrow = 2, dir = "v") + 
  geom_boxplot() +
  geom_boxplot_pattern(position = position_dodge(preserve = "single"), color = "black", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6) + 
  ...
  theme(axis.text.x=element_text(size=20, angle = 90, vjust = 0.5, hjust=1),
        ...
        strip.text = element_text(size = 20))
# 2019 #
plot.2019 <- mean_daily_new %>% 
filter(datetimeAK < "2020-01-01" & datetimeAK > "2019-01-01") %>%
ggplot(aes(x = site.ID, y = dailyNO3, pattern = PF, fill = site.ID)) +
  facet_wrap(~year, nrow = 2, dir = "v") + 
  geom_boxplot() +
  geom_boxplot_pattern(position = position_dodge(preserve = "single"), color = "black", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6) + 
...
# 2020 #
plot.2020 <- mean_daily_new %>% 
filter(datetimeAK < "2021-01-01" & datetimeAK > "2020-01-01") %>%
ggplot(aes(x = site.ID, y = dailyNO3, pattern = PF, fill = site.ID)) +
  facet_wrap(~year, nrow = 2, dir = "v") + 
  geom_boxplot() +
  geom_boxplot_pattern(position = position_dodge(preserve = "single"), color = "black", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6) + 
...
# 2022 #
plot.2022 <- mean_daily_new %>% 
filter(datetimeAK < "2022-01-01" & datetimeAK > "2021-01-01") %>%
ggplot(aes(x = site.ID, y = dailyNO3, pattern = PF, fill = site.ID)) +
  facet_wrap(~year, nrow = 2, dir = "v") + 
  geom_boxplot() +
  geom_boxplot_pattern(position = position_dodge(preserve = "single"), color = "black", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6) + 
...
no3.plot <- plot_layout(plot.2015 + plot.2018 + plot.2019 +
            plot.2020 + plot.2021 + plot.2022, 
            nrow = 2, ncol = 3)


# THESE ARE THE ONES THAT I PRINTED IN MY LATEST DRAFT TO TKH -02/10/2023
vn = expression(paste(N*O[3]^"-"*(μM)))
...
CV_mean_conc$PF <- NULL

CV_mean_conc$PF <- ifelse(CV_mean_conc$site.ID == "VAUL"| CV_mean_conc$site.ID == "STRT",
                       "High", "Moderate")

CV_mean_conc$site.ID <- factor(CV_mean_conc$site.ID,     # Reorder factor levels
                         c("CARI", "FRCH", "VAUL", "MOOS", "POKE", "STRT"))



ggplot(CV_mean_conc, aes(x = site.ID, y = CVconc, pattern = PF, fill = site.ID)) +
  geom_boxplot() + 
  geom_boxplot_pattern(position = position_dodge(preserve = "single"), color = "black", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6) +
  scale_pattern_manual(values = c(High = "stripe", Moderate = "none")) +
  scale_y_continuous(trans='log10') +
  scale_fill_manual(values=c("#3288BD","#FF7F00", "#E7298A", "#A6761D", "#6A3D9A", "#66C2A5"), "Site") +
  xlab("") +
  ylab("") +
  theme_classic() +
