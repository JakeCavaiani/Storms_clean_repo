...
ggplot(mean_daily_new, aes(x = site.ID, y = dailyNO3, pattern = Burn, fill = site.ID)) +
  ...
  scale_pattern_manual(values = c(burned = "stripe", unburned = "none")) +
  scale_fill_manual(values=c("#3288BD","#FF7F00", "#E7298A", "#A6761D", "#6A3D9A", "#66C2A5"), guide = "none") +
  geom_signif(comparisons = list(c("CARI", "FRCH"), c("CARI", "VAUL"), c("CARI", "MOOS"), c("CARI", "POKE"), c("CARI", "STRT"),
                                 c("FRCH", "VAUL"), c("FRCH", "MOOS"), c("FRCH", "POKE"), c("FRCH", "STRT"),
                                 c("VAUL", "MOOS"), c("VAUL", "POKE"), c("VAUL", "STRT"),
                                 c("MOOS", "POKE"), c("MOOS", "STRT"),
                                 c("POKE", "STRT")), 
              map_signif_level = TRUE,
              step_increase = 0.2) +
...
ggplot(mean_daily_new, aes(x = site.ID, y = dailyNO3, pattern = Burn, fill = site.ID)) +
  ...
  facet_wrap(~year) +
  geom_text(data = annotations, aes(label = Label, y = y_position, pattern = Burn), 
...
# Define the labels and positions for annotation
annotations <- data.frame(
  Facet = c("X", "X", "Y", "Y"),
  Group = c("A", "B", "C", "D"),
  Label = c("a", "b", "c", "d"),
  y_position = c(16, 14, 22, 20)
)
